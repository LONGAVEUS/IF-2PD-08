            ...
            <tbody class="divide-y divide-indigo-50 text-sm">
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center">
                        <span
                            class="px-3 py-1 rounded-full text-[10px] font-bold {{ $d->status == 'aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ strtoupper(str_replace('_', ' ', $d->status)) }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button data-modal-target="modalEdit-{{ $d->id }}"
                                data-modal-toggle="modalEdit-{{ $d->id }}"
                ...
                <x-modal id="modalEdit-{{ $d->id }}" title="Edit Data Dosen">
                    <form action="{{ route('dosen.update', $d->id) }}" method="POST" class="p-0.5">
                        ...
                        <div class="mb-5">
                            <label
                                class="block mb-1.5 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Jurusan:</label>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-7">
                            <div>
                                <label
                                    class="block mb-1.5 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Password:</label>
                                <input type="password" name="password" placeholder="Kosongkan jika tetap"
                                    class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 p-3 outline-none transition-all">
                            </div>
                            <div>
                                <label
                                    class="block mb-1.5 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Status:</label>
                                <select name="status"
                                    ...
                                    <option value="tidak_aktif" {{ $d->status == 'tidak_aktif' ? 'selected' : '' }}>
                                        Tidak Aktif</option>
                                </select>
                            </div>
                        </div>
...
<x-modal id="modalTambahDosen" title="Tambah Dosen">
    <form action="{{ route('dosen.store') }}" method="POST" class="p-1 space-y-4">
        ...
        <button type="submit"
            class="w-full bg-indigo-600 text-white font-bold py-3.5 rounded-2xl active:scale-95 transition-all">Simpan</button>
    </form>
</x-modal>
@endsection
