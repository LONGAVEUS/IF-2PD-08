...
<div class="max-w-7xl mx-auto space-y-6 pb-8">
            ...
            <tbody class="divide-y divide-indigo-50 text-sm">
                ...
                <tr class="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td class="px-6 py-4 text-center">
                        <span class="px-3 py-1 rounded-full text-[10px] font-bold {{ $m->status == 'aktif' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' }}">
                            {{ strtoupper($m->status) }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-center gap-2">
                            <button data-modal-target="modalEdit-{{ $m->id }}"
                                ...
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path d="M11 5H6a2 2 0 0 0-2 2v11a2 2 0 0 0 2 2h11a2 2 0 0 0 2-2v-5M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                                </svg>
                            </button>
                ...
                <x-modal id="modalEdit-{{ $m->id }}" title="Edit Data Mahasiswa">
                    <form action="{{ route('mahasiswa.update', $m->id) }}" method="POST" class="p-0.5">
                        ...
                        @method('PUT')
                        <div class="mb-5">
                            <label class="block mb-2 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">NIM:</label>
                        ...
                        <div class="mb-5">
                            <label class="block mb-2 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Nama Lengkap Mahasiswa:</label>
                            <input type="text" name="name" value="{{ $m->name }}" required
                                class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 p-3 outline-none transition-all">
                        </div>
                        <div class="mb-5">
                            <label class="block mb-2 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Program Studi:</label>
                            <select name="prodi" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none focus:border-indigo-500">
                                <option value="Informatika" {{ $m->mahasiswa->prodi == 'Informatika' ? 'selected' : '' }}>Informatika</option>
                                <option value="Sistem Informasi" {{ $m->mahasiswa->prodi == 'Sistem Informasi' ? 'selected' : '' }}>Sistem Informasi</option>
                            </select>
                        </div>
                        <div class="mb-5">
                            <label class="block mb-2 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Ganti Password (Opsional):</label>
                            <input type="password" name="password" placeholder="Kosongkan jika tidak ingin ganti"
                                class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 p-3 outline-none transition-all">
                        </div>
                        <div class="grid grid-cols-2 gap-4 mb-7">
                            <div>
                                <label class="block mb-2 text-[11px] font-bold text-indigo-900 uppercase tracking-wider">Semester:</label>
                                <select name="semester_ke" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none">
                                    <option value="1" {{ $m->mahasiswa->semester_ke == 1 ? 'selected' : '' }}>Semester 1</option>
                                    <option value="2" {{ $m->mahasiswa->semester_ke == 2 ? 'selected' : '' }}>Semester 2</option>
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="status" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none font-bold text-indigo-600">
                                    ...
                                    <option value="tidak_aktif" {{ $m->status == 'tidak_aktif' ? 'selected' : '' }}>Tidak Aktif</option>
                                </select>
                            </div>
                        </div>
                        <button type="submit"
    ...
    <x-modal id="modalTambah" title="Tambah Mahasiswa">
        <form action="{{ route('mahasiswa.store') }}" method="POST" class="p-0.5">
            ...
            <div class="mb-5">
                <label class="block mb-1.5 text-[11px] font-bold text-gray-600 uppercase tracking-wider">Nama Mahasiswa:</label>
                <input type="text" name="name" placeholder="Nama Lengkap Mahasiswa" required
                    class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 p-3 outline-none transition-all">
            </div>
            <div class="mb-5">
                <label class="block mb-1.5 text-[11px] font-bold text-gray-600 uppercase tracking-wider">Program Studi:</label>
                <select name="prodi" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none focus:border-indigo-500 appearance-none">
                    <option value="">Pilih Program Studi</option>
                    <option value="Informatika">Informatika</option>
                </select>
            </div>
            <div class="mb-5">
                <label class="block mb-1.5 text-[11px] font-bold text-gray-600 uppercase tracking-wider">Password Login:</label>
                <input type="password" name="password" placeholder="Buat Password Akun" required
                    class="w-full bg-white border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 p-3 outline-none transition-all">
            </div>
            <div class="grid grid-cols-2 gap-4 mb-7">
                <div>
                    <label class="block mb-1.5 text-[11px] font-bold text-gray-600 uppercase tracking-wider">Semester:</label>
                    <select name="semester_ke" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none">
                ...
                <div>
                    <label class="block mb-1.5 text-[11px] font-bold text-gray-600 uppercase tracking-wider">Status:</label>
                    <select name="status" required class="w-full bg-white border border-gray-200 text-sm rounded-xl p-3 outline-none font-bold text-indigo-600">
