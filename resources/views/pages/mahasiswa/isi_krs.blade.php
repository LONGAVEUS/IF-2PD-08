...
<div class="space-y-6">
    ...
    {{-- PAGE HEADER --}}
    <div
        class="bg-white rounded-2xl border border-slate-100 shadow-sm px-7 py-6 flex flex-col md:flex-row md:items-center md:justify-between gap-3 animate-[fadeUp_0.42s_ease_both]">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between px-5 py-5 border-b border-slate-100 gap-4">
        <div>
            ...
            <p class="text-[10px] md:text-xs text-slate-400 mt-0.5">
                ...
                &nbsp;·&nbsp;
                <span
                    class="font-semibold text-slate-600">{{ $mataKuliahTerdaftar->sum(fn($k) => $k->mata_kuliah->sks) }}
                    SKS</span>
            </p>
        </div>
    ...
    <div class="overflow-x-auto">
        <div
            class="bg-white border-2 border-indigo-50 rounded-2xl overflow-hidden shadow-lg shadow-indigo-500/5 overflow-x-auto">
            <table class="w-full text-left min-w-[800px] border-collapse">
                <thead class="bg-indigo-50/50">
                    <tr>
                        <th
                            class="text-xs font-semibold uppercase tracking-wider text-indigo-800 px-5 py-4 border-b border-indigo-100 w-16">
                            NO</th>
                        <th
                            class="text-xs font-semibold uppercase tracking-wider text-indigo-800 px-5 py-4 border-b border-indigo-100">
                            Kode MK</th>
                        <th
                            class="text-xs font-semibold uppercase tracking-wider text-indigo-800 px-5 py-4 border-b border-indigo-100">
                            Nama Mata Kuliah</th>
                        ...
                        <th
                            ...
                            Dosen</th>
                        <th
                            class="text-xs font-semibold uppercase tracking-wider text-indigo-800 px-5 py-4 border-b border-indigo-100 text-center">
                            Aksi</th>
                    </tr>
                </thead>
        ...
        <div class="relative bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="flex items-center justify-between p-5 border-b">
                ...
                <button type="button" class="text-slate-400 hover:text-slate-600"
                    data-modal-hide="modalPilihMK">✕</button>
            </div>
            <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div class="flex justify-between items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
                    ...
                    <form action="{{ route('mahasiswa.krs.tambah') }}" method="POST">
                        ...
                        <button type="submit"
                            class="bg-indigo-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-indigo-700">Tambah</button>
                    </form>
                </div>
                @endforeach
            </div>
