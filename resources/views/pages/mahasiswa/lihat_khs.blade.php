...
<div class="flex flex-col sm:flex-row sm:items-center gap-4 mb-4">
    ...
    <div class="w-full sm:w-auto">
        <form action="{{ route('lihat_khs') }}" method="GET">
        ...
        <table class="w-full text-left border-collapse min-w-max">
            ...
            <tr class="hover:bg-indigo-50/30 transition-colors">
                ...
                <td class="px-5 py-4 align-middle">
                    <span class="text-sm font-medium text-gray-500">{{ $n->krs->mata_kuliah->kode_mk ?? '-' }}</span>
                </td>
                <td class="px-5 py-4 align-middle font-semibold text-gray-800">{{ $n->krs->mata_kuliah->nama_mk ?? '-' }}</td>
                <td class="px-5 py-4 align-middle font-semibold text-gray-800">{{ $n->krs->mata_kuliah->sks ?? 0 }}</td>
                <td class="px-5 py-4 align-middle">
                    <span class="text-sm font-bold text-indigo-700" id="huruf-{{ $user->mahasiswa->nim }}">
                        {{ $n->nilai_huruf ?? '-' }} {{-- <-- Jika null, cetak '-' --}}
                    </span>
                </td>

                <td class="px-5 py-4 align-middle font-semibold text-gray-800">
                    {{ isset($n->bobot) ? (float)$n->bobot : '-' }}
                </td>

                <td class="px-5 py-4 align-middle font-semibold text-gray-800">
                    {{ isset($n->bobot) ? (float)(($n->krs->mata_kuliah->sks ?? 0) * $n->bobot) : '-' }}
                </td>
            </tr>
            @endforeach
        </tbody>
